...
class BruteForcePlatform:
    @staticmethod
    def execute(pid_args: Tuple[float, float, float], mass: float, delta_t, setpoints: List[float], error_fun: Callable, weight_factor: float):
        kp, ki, kd = pid_args
        ...
        error = agent.run(setpoints, weight_factor)

        return kp, ki, kd, error
